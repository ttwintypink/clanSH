...
Важно для хостинга:
  - Токен берём ТОЛЬКО из переменных окружения (ENV).
  - Файлы .env / dotenv намеренно не используются, чтобы не было путаницы с панелью,
    где токен задаётся в настройках окружения.
"""
...
def _clean_token(raw: str) -> str:
    """Чистим токен от пробелов/кавычек, которые часто добавляют панели хостингов."""
    v = raw.strip()
...
# Поддерживаем популярные названия переменных в разных панелях/хостингах
...
if not TOKEN:
    raise RuntimeError(
        ...
        "Задай ОДНУ из переменных в настройках хостинга (ENV):\n"
        "  DISCORD_TOKEN  или  TOKEN  или  BOT_TOKEN  или  DISCORD_BOT_TOKEN\n"
    )

# Неболтливый маркер (без токена) — помогает понять, откуда он взят
