...
async def on_guild_channel_create(channel):
    if isinstance(channel, discord.TextChannel) and channel.category_id == TICKETS_CATEGORY_ID:
        ...
        # 2) всегда пытаемся отправить панель модератора (кнопки принятия/отклонения)
        # даже если приветствие не отправилось.
        await ensure_decision_prompt(channel, reason="channel_create")
