...
async def _resolve_display_name(interaction: discord.Interaction, user_id: int) -> str:
    ...
    # 1) Пытаемся взять из кэша гильдии (мгновенно)
    if interaction.guild:
        m = interaction.guild.get_member(user_id)
        if m:
            return m.display_name

        # 2) Если не в кэше — fetch_member (HTTP). Это может быть медленно,
        # поэтому используем таймаут, чтобы /menu не "висел".
        try:
            m2 = await asyncio.wait_for(interaction.guild.fetch_member(user_id), timeout=1.8)
...
async def menu_cmd(interaction: discord.Interaction):
    ...
    # /menu может потребовать несколько HTTP запросов, поэтому быстро ACK'аем интеракцию
    ...
    if not all_ids:
        await interaction.followup.send("(список исключений пуст)", ephemeral=True)
        return

    # Строим строки: id - display_name
    # Небольшая параллельность, чтобы не ждать по одному fetch_member
    sem = asyncio.Semaphore(5)

    async def one(uid: int) -> str:
        async with sem:
            name = await _resolve_display_name(interaction, uid)
            return f"{uid} - {name}"

    lines = list(await asyncio.gather(*(one(uid) for uid in all_ids)))
