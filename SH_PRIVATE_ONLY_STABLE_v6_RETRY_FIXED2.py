...
SYNC_COMMANDS_TO_ALL_GUILDS = False



# ВАЖНО: slash-команды должны быть ТОЛЬКО в приватке.
# Если раньше ты синкал команды глобально, Discord может продолжать показывать их
# на всех серверах. Поставь True на ОДИН запуск, чтобы стереть глобальные команды,
# затем можно вернуть False.
DELETE_GLOBAL_COMMANDS_ON_START = True
...
@client.event
async def on_ready():
    """Старт бота: команды синкаем только в приватный сервер."""
    try:
    ...
    # 1) Опционально удаляем глобальные slash-команды, чтобы они НЕ появлялись в паблике
    if DELETE_GLOBAL_COMMANDS_ON_START:
        try:
            tree.clear_commands(guild=None)
            await tree.sync()  # глобальный sync с пустым набором = удаление
            print("🧹 Global application commands cleared (private-only mode).")
        except Exception as e:
            print(f"⚠️  Failed to clear global commands: {type(e).__name__}: {e}")

    # 2) Синк в приватке (появляется почти сразу)
    try:
        synced = await tree.sync(guild=discord.Object(id=PRIVATE_GUILD_ID))
        print(f"🔁 [SYNC] private guild ({PRIVATE_GUILD_ID}): {len(synced)} command(s)")
    except Exception as e:
        print(f"❌ [SYNC] private guild failed: {type(e).__name__}: {e}")
...
async def on_guild_join(guild):
    # Команды приватные: на новых серверах ничего не синкаем
    return
...
def _safe_run_forever():
    ...
    while True:
        try:
            ...
            print("\n" + "=" * 72)
            print("❌ Бот не смог подключиться к Discord (таймаут/сеть).")
            print("=" * 72)
            print(f"Причина: {type(e).__name__}: {e}")
            traceback.print_exc()
            print("=" * 72)
            sleep_for = min(backoff, 120)
            print(f"🔁 Повторная попытка через {sleep_for} сек...")
