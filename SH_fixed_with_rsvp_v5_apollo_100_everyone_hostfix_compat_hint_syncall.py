...
async def on_ready():
    ...
    # синк slash-команд
    # Важно: если бот на нескольких серверах, а синк сделан только в одну гильдию,
    # то в остальных команда в меню / не появятся.
    try:
        if SYNC_COMMANDS_TO_ALL_GUILDS:
            ...
            for g in client.guilds:
                try:
                    ...
                    print(f"[WARN] Command sync failed for guild {g.id}: {e}")
                await asyncio.sleep(0.25)  # чуть притормозим, чтобы не упереться в rate-limit
            print(f"[SYNC] Commands synced to {ok}/{total} guild(s)")
        else:
            # быстро в конкретную гильдию (если указано) или глобально (может занять время до появления)
            if COMMANDS_SYNC_GUILD_ID:
                await tree.sync(guild=discord.Object(id=COMMANDS_SYNC_GUILD_ID))
                print(f"[SYNC] Commands synced to guild {COMMANDS_SYNC_GUILD_ID}")
            else:
                await tree.sync()
