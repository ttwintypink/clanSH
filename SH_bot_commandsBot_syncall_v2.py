...
# Синкать команды во ВСЕ гильдии, где есть бот (чтобы они появлялись сразу в каждом сервере)
SYNC_COMMANDS_TO_ALL_GUILDS = True
...
# --- Bot bootstrap (commands.Bot) ---
bot = commands.Bot(command_prefix="!", intents=intents)
# Backward-compat: the rest of the file still uses `client`/`tree`
client = bot
tree = bot.tree
...
# ---- Aliases to avoid collisions / make commands easy to find ----
@tree.command(
    name="sh_kanal",
    description="(SH) Указать канал для событий регистрации",
)
@app_commands.describe(channel="Канал, куда бот будет отправлять объявления/меню")
async def cmd_set_channel_sh(interaction: discord.Interaction, channel: discord.TextChannel):
    # Просто вызываем основную команду
    await cmd_set_channel(interaction, channel)


@tree.command(
    name="sh_sobytie",
    description="(SH) Создать событие регистрации (все шаги придут в ЛС)",
)
async def cmd_create_event_sh(interaction: discord.Interaction):
    # Просто вызываем основную команду
    await cmd_create_event(interaction)
...
async def cmd_create_event(interaction: discord.Interaction):
    ...
    if not ch_id:
        return await interaction.response.send_message("Сначала укажи канал командой /kanal.", ephemeral=True)
...
@client.event
async def on_ready():
    # We log in + aggressively sync commands so they appear instantly in guild(s).
    # IMPORTANT: In discord.py, syncing to a guild requires the commands to exist in the guild scope.
    # So we: clear guild commands -> copy global commands -> sync guild.
    try:
        print(f"✅ Logged in as {client.user} (id={client.user.id})")
    except Exception:
        print("✅ Logged in.")

    await ensure_assets_dir()
    await ensure_data_dir()
    load_data()

    async def _sync_one_guild(gid: int, gname: str = "") -> bool:
        guild_obj = discord.Object(id=gid)
        try:
            # Make guild commands mirror global commands (fast propagation)
            try:
            ...
            try:
                ...
                # Older discord.py: copy_global_to may not exist; fallback to global sync only
                pass

            synced = await tree.sync(guild=guild_obj)
            print(f"🔁 [SYNC] {gname or 'guild'} ({gid}): {len(synced)} command(s)")
            return True
        except Exception as e:
            print(f"❌ [SYNC] {gname or 'guild'} ({gid}) failed: {type(e).__name__}: {e}")
            return False

    if SYNC_COMMANDS_TO_ALL_GUILDS:
        ok = 0
        total = 0
        for g in client.guilds:
            total += 1
            if await _sync_one_guild(g.id, getattr(g, "name", "")):
                ok += 1
            await asyncio.sleep(0.25)

        # Also do a global sync (can take time to propagate, but harmless)
        try:
            global_synced = await tree.sync()
            print(f"🌍 [GLOBAL SYNC] {len(global_synced)} command(s) pushed globally (may take time to appear).")
        except Exception as e:
            print(f"⚠️  [GLOBAL SYNC] skipped/failed: {type(e).__name__}: {e}")

        print(f"✅ Sync done: {ok}/{total} guild(s) updated.")
    elif COMMANDS_SYNC_GUILD_ID:
        await _sync_one_guild(COMMANDS_SYNC_GUILD_ID, "configured")
        try:
            global_synced = await tree.sync()
            print(f"🌍 [GLOBAL SYNC] {len(global_synced)} command(s) pushed globally (may take time to appear).")
        except Exception:
    ...
    # status
    await client.change_presence(activity=discord.Game(name=" /kanal • /sobytie"))

@client.event
async def on_guild_join(guild):
    # When the bot joins a new server, sync commands there immediately.
    try:
        ...
        try:
            ...
            pass
        synced = await tree.sync(guild=guild_obj)
        print(f"🔁 [SYNC] joined {getattr(guild,'name','guild')} ({guild.id}): {len(synced)} command(s)")
    except Exception as e:
        print(f"❌ [SYNC] on_guild_join failed for {getattr(guild,'id','?')}: {type(e).__name__}: {e}")
...
async def on_message(message: discord.Message):
    ...
    # если пользователь просто отправил текст "/kanal" или "/sobytie" как обычное сообщение —
    # подскажем, что нужно выбрать slash-команду из меню (через /), иначе бот не получит Interaction.
    if message.content:
        raw = message.content.strip()
        if raw in ("/kanal", "/sobytie"):
            try:
                await message.reply(
                    "Это **slash-команды**.\n"
                    "Нажми в поле ввода **/** и выбери **kanal** или **sobytie** из списка (не отправляй как обычное сообщение).\n"
                    "Если команд в списке нет — проверь права **Use Application Commands** и что бот приглашён со scope **applications.commands**.",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
            except discord.HTTPException:
                pass
    ...
    # Let prefix-commands (if any) work even with a custom on_message
    try:
